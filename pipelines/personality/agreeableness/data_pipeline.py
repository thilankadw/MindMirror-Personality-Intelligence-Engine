"""Data pipeline for agreeableness."""
import os
import json
import logging
import pandas as pd
...
from utils.config import (
    ...
    get_mlflow_config,
    get_agreeableness_direction_config,
)
...
def data_pipeline(
    ...
    direction_cfg = get_agreeableness_direction_config()
    ...
    if (
        ...
        try:
            ...
            expected_features = [
                "agreeableness_lag1",
                "agreeableness_roll4",
                "agreeableness_roll4_std",
            ]
            missing_features = [c for c in expected_features if c not in X_train_df.columns]
            label_ok = label_col in Y_train_df.columns

            if missing_features or not label_ok:
                logger.warning(
                    "Existing artifacts do not match agreeableness schema. "
                    "missing_features=%s label_ok=%s. Regenerating artifacts.",
                    missing_features,
                    label_ok,
                )
            else:
    ...
    logger.info("Preparing agreeableness direction dataset...")
    ...
    # Build agreeableness working dataframe
    optional_cols = [
        c
    ...
    _log_df_state(df_o, "agreeableness_subset")
    ...
    if "agreeableness_lag1" not in df_o.columns:
        df_o["agreeableness_lag1"] = df_o[obs_col]
    _log_df_state(df_o, "agreeableness_lag1_base")
    ...
    rolling_creator = RollingStatsFeatureCreator(
        ...
        window=4,
        min_periods=2,
        mean_column="agreeableness_roll4",
        std_column="agreeableness_roll4_std",
    )
    ...
    numeric_features_cp = ["agreeableness_lag1", "agreeableness_roll4", "agreeableness_roll4_std"]
    ...
    try:
        ...
        run_tags = create_mlflow_run_tags(
            "data_pipeline",
            {
                ...
                "target_type": "agreeableness_direction",
                "num_targets": "1",
            },
        )
        run_name = (
