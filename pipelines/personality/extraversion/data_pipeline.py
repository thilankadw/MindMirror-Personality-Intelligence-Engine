"""Data pipeline for extraversion."""
import os
import json
import logging
import pandas as pd
...
from utils.config import (
    ...
    get_traits,
    get_extraversion_direction_config,
    get_mean_features,
    get_std_features,
...
def data_pipeline(
    ...
    direction_cfg = get_extraversion_direction_config()
    ...
    logger.info("Preparing extraversion direction dataset...")
    ...
    has_trait_cols = all(str(trait).lower() in df_cols_lower for trait in traits)
    ...
    if not is_weekly:
        ...
        try:
            if analysis_traits:
                df_cols_lower = {str(c).lower() for c in df.columns}
                missing_traits = [
                    t for t in analysis_traits if str(t).lower() not in df_cols_lower
                ]
                if missing_traits:
                    logger.warning(
    ...
    # Build extraversion working dataframe
    optional_cols = [
        c
    ...
    df_e = df[keep_cols].copy()
    _log_df_state(df_e, "extraversion_subset")
    ...
    df_e = emb_parser.parse(df_e)
    _log_df_state(df_e, "embedding_parse")
    ...
    df_e = gap_calc.calculate(df_e)
    _log_df_state(df_e, "time_gap_features")
    ...
    df_e = delta_creator.create(df_e, groupby_col=author_col)
    _log_df_state(df_e, "future_delta_targets")

    # Remove rows without sufficient context
    before_rows = df_e.shape[0]
    df_e = df_e.dropna(subset=["past_mean", "future_mean", "delta_future"]).copy()
    after_rows = df_e.shape[0]
    logger.info("Context filter: rows before=%s after=%s", before_rows, after_rows)
    _log_df_state(df_e, "context_filtered")
    ...
    df_e = splitter.add_split_column(df_e, column="is_train")
    _log_df_state(df_e, "author_forward_split")
    ...
    df_e = labeler.fit_transform(df_e)
    logger.info("Tau used for labels: %.6f", labeler.tau_)
    _log_df_state(df_e, "direction_labels")

    # Feature construction
    if "extraversion_lag1" not in df_e.columns:
        df_e["extraversion_lag1"] = df_e[obs_col]
    _log_df_state(df_e, "extraversion_lag1_base")
    ...
    rolling_creator = RollingStatsFeatureCreator(
        ...
        window=4,
        min_periods=2,
        mean_column="extraversion_roll4",
        std_column="extraversion_roll4_std",
    )
    df_e = rolling_creator.create(df_e)
    _log_df_state(df_e, "rolling_stats")
    ...
    numeric_features = [
        ...
        "extraversion_lag1",
        "extraversion_roll4",
        "extraversion_roll4_std",
    ]
    numeric_features_cp = ["extraversion_lag1", "extraversion_roll4", "extraversion_roll4_std"]

    if std_col:
        if std_col not in df_e.columns:
            raise ValueError(
    ...
    df_e = shift_creator.create(df_e)
    df_e["emb_vec_shift"] = df_e.groupby(author_col)["emb_vec"].shift(1)
    _log_df_state(df_e, "feature_shift")

    required = numeric_features + ["emb_vec_shift", label_col, "is_train"]
    df_e = df_e.sort_values([author_col, time_sort_col]).reset_index(drop=True)
    model_df = df_e.dropna(subset=required).copy()
    ...
    try:
        ...
        run_tags = create_mlflow_run_tags(
            "data_pipeline",
            {
                ...
                "target_type": "extraversion_direction",
                "num_targets": "1",
            },
        )
        run_name = (
