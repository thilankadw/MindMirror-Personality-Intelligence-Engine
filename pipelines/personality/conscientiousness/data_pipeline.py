"""Data pipeline for conscientiousness."""
import os
import json
import logging
import pandas as pd
...
from utils.config import (
    ...
    get_traits,
    get_conscientiousness_direction_config,
    get_mean_features,
    get_std_features,
...
def data_pipeline(
    ...
    direction_cfg = get_conscientiousness_direction_config()
    ...
    lower_obs = obs_col.lower()
    trait_base = obs_col
    if lower_obs.endswith("_mean_w"):
        trait_base = obs_col[: -len("_mean_w")]
    elif lower_obs.endswith("_mean"):
        trait_base = obs_col[: -len("_mean")]
    trait_label = trait_base
    trait_slug = trait_base.lower()
    ...
    logger.info("Preparing %s direction dataset...", trait_label)
    ...
    _log_df_state(df_c, f"{trait_slug}_subset")
    ...
    df_c = emb_parser.parse(df_c)
    _log_df_state(df_c, "embedding_parse")
    ...
    df_c = gap_calc.calculate(df_c)
    _log_df_state(df_c, "time_gap_features")
    ...
    df_c = delta_creator.create(df_c, groupby_col=author_col)
    _log_df_state(df_c, "future_delta_targets")

    # Remove rows without sufficient context
    before_rows = df_c.shape[0]
    df_c = df_c.dropna(subset=["past_mean", "future_mean", "delta_future"]).copy()
    after_rows = df_c.shape[0]
    logger.info("Context filter: rows before=%s after=%s", before_rows, after_rows)
    _log_df_state(df_c, "context_filtered")
    ...
    df_c = splitter.add_split_column(df_c, column="is_train")
    _log_df_state(df_c, "author_forward_split")
    ...
    df_c = labeler.fit_transform(df_c)
    logger.info("Tau used for labels: %.6f", labeler.tau_)
    _log_df_state(df_c, "direction_labels")

    # Feature construction
    lag_feature = f"{trait_slug}_lag1"
    roll_mean_feature = f"{trait_slug}_roll4"
    roll_std_feature = f"{trait_slug}_roll4_std"
    if lag_feature not in df_c.columns:
        df_c[lag_feature] = df_c[obs_col]
    _log_df_state(df_c, f"{trait_slug}_lag1_base")
    ...
    rolling_creator = RollingStatsFeatureCreator(
        ...
        window=4,
        min_periods=2,
        mean_column=roll_mean_feature,
        std_column=roll_std_feature,
    )
    df_c = rolling_creator.create(df_c)
    _log_df_state(df_c, "rolling_stats")
    ...
    numeric_features_cp = [lag_feature, roll_mean_feature, roll_std_feature]

    if std_col:
        if std_col not in df_c.columns:
            raise ValueError(
    ...
    df_c = shift_creator.create(df_c)
    df_c["emb_vec_shift"] = df_c.groupby(author_col)["emb_vec"].shift(1)
    _log_df_state(df_c, "feature_shift")

    required = numeric_features + ["emb_vec_shift", label_col, "is_train"]
    df_c = df_c.sort_values([author_col, time_sort_col]).reset_index(drop=True)
    model_df = df_c.dropna(subset=required).copy()
    ...
    try:
        ...
        run_tags = create_mlflow_run_tags(
            "data_pipeline",
            {
                ...
                "target_type": (
                    label_col.replace("_dir", "_direction")
                    if label_col.endswith("_dir")
                    else label_col
                ),
                "num_targets": "1",
            },
