...
from utils.config import (
    ...
    get_traits,
    get_neuroticism_direction_config,
    get_mean_features,
    get_std_features,
...
def data_pipeline(
    ...
    direction_cfg = get_neuroticism_direction_config()
    ...
    logger.info("Preparing neuroticism direction dataset...")
    ...
    # Build neuroticism working dataframe
    optional_cols = [
        c
    ...
    df_n = df[keep_cols].copy()
    _log_df_state(df_n, "neuroticism_subset")
    ...
    df_n = emb_parser.parse(df_n)
    _log_df_state(df_n, "embedding_parse")
    ...
    df_n = gap_calc.calculate(df_n)
    _log_df_state(df_n, "time_gap_features")
    ...
    df_n = delta_creator.create(df_n, groupby_col=author_col)
    _log_df_state(df_n, "future_delta_targets")

    # Remove rows without sufficient context
    before_rows = df_n.shape[0]
    df_n = df_n.dropna(subset=["past_mean", "future_mean", "delta_future"]).copy()
    after_rows = df_n.shape[0]
    logger.info("Context filter: rows before=%s after=%s", before_rows, after_rows)
    _log_df_state(df_n, "context_filtered")
    ...
    df_n = splitter.add_split_column(df_n, column="is_train")
    _log_df_state(df_n, "author_forward_split")
    ...
    df_n = labeler.fit_transform(df_n)
    logger.info("Tau used for labels: %.6f", labeler.tau_)
    _log_df_state(df_n, "direction_labels")

    # Feature construction
    if "neuroticism_lag1" not in df_n.columns:
        df_n["neuroticism_lag1"] = df_n[obs_col]
    _log_df_state(df_n, "neuroticism_lag1_base")
    ...
    rolling_creator = RollingStatsFeatureCreator(
        ...
        window=4,
        min_periods=2,
        mean_column="neuroticism_roll4",
        std_column="neuroticism_roll4_std",
    )
    df_n = rolling_creator.create(df_n)
    _log_df_state(df_n, "rolling_stats")
    ...
    numeric_features = [
        ...
        "neuroticism_lag1",
        "neuroticism_roll4",
        "neuroticism_roll4_std",
    ]
    numeric_features_cp = ["neuroticism_lag1", "neuroticism_roll4", "neuroticism_roll4_std"]

    if std_col:
        if std_col not in df_n.columns:
            raise ValueError(
    ...
    df_n = shift_creator.create(df_n)
    df_n["emb_vec_shift"] = df_n.groupby(author_col)["emb_vec"].shift(1)
    _log_df_state(df_n, "feature_shift")

    required = numeric_features + ["emb_vec_shift", label_col, "is_train"]
    df_n = df_n.sort_values([author_col, time_sort_col]).reset_index(drop=True)
    model_df = df_n.dropna(subset=required).copy()
    ...
    try:
        ...
        run_tags = create_mlflow_run_tags(
            "data_pipeline",
            {
                ...
                "target_type": "neuroticism_direction",
                "num_targets": "1",
            },
        )
        run_name = (
